...
export default class VuePrintNext {
  ...
  private previewIframeLoad() {
    ...
    this.addEvent(box.querySelector('.previewBodyUtilPrintBtn'), 'click', () => {
      ...
      if (iframe?.contentWindow) {
        // 预览模式下也等待字体加载完成
        this.waitForFontsLoaded(iframe.contentWindow).then(() => {
          this.settings.openCallback?.();
          iframe.contentWindow?.print();
          this.settings.closeCallback?.();
        }).catch(() => {
          console.warn(`${FUNC_NAME}: Font loading timeout in preview mode, proceeding with print`);
          this.settings.openCallback?.();
          iframe.contentWindow?.print();
          this.settings.closeCallback?.();
        });
  ...
  private print(iframe: PrintAreaWindow) {
    ...
    const _loaded = () => {
      // 等待字体加载完成后再打印
      this.waitForFontsLoaded(iframeWin).then(() => {
        ...
        // 如果字体加载失败或超时，仍然执行打印
        console.warn(`${FUNC_NAME}: Font loading timeout, proceeding with print`);
        const timer = setTimeout(() => {
          iframeWin.focus();
          this.settings.openCallback?.();
          iframeWin.print();
  ...
  private createIframe(url: string): HTMLIFrameElement {
    ...
    // 设置iframe初始样式为不可见
    // 注意: 不要设置display:none,否则可能导致字体无法正确加载
    // 参考: https://github.com/Alessandro-Pang/vue-print-next/issues/19 
    iframe.style.cssText = 'position: fixed; left: -9999px; border: 0; width: 0; height: 0; opacity: 0;';
   ...
   */
  private waitForFontsLoaded(targetWindow: Window): Promise<void> {
    return new Promise((resolve, reject) => {
      const timeout = 3000; // 3秒超时
      const startTime = Date.now();
      
      // 检查是否支持document.fonts API
      if (targetWindow.document.fonts && typeof targetWindow.document.fonts.ready === 'object') {
        Promise.race([
          targetWindow.document.fonts.ready,
          new Promise((_, timeoutReject) => {
            setTimeout(() => timeoutReject(new Error('Font loading timeout')), timeout);
          })
        ]).then(() => {
          resolve();
        }).catch(() => {
          reject(new Error('Font loading timeout or failed'));
        });
      } else {
        // 降级方案：检查字体是否已加载
